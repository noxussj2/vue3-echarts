<template>
    <div class="echarts-next__pie-simple">
        ...
        <div class="pie-simple__hover" :style="{ left: props.center[0] }" v-if="props.data.length">
            <div class="pie-count">
                <div class="count-number">
                    <div class="number">{{ props.data[activeIndex].value }}</div>
                    <div class="title">{{ props.data[activeIndex].name }} {{ props.data[activeIndex].unit ? `(${props.data[activeIndex].unit})` : '' }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="pie-simple__legend">
            <div class="legend__item" v-for="(item, index) in props.data" :key="index">
                ...
                <span class="item-percent">{{ ((item.value / count) * 100).toFixed(2) }}%</span>
            </div>
        </div>
    </div>
</template>
        ...
        async () => {
            if (props.data.length) {
                ...
                timer = setInterval(() => {
                    ...
                    if (activeIndex.value === props.data.length) {
                        activeIndex.value = 0;
                    }
                }, 1000 * 10);
            }
